...
(function (factory) {
  ...
  var dialog = renderer.create('<div class="modal modal-closed" aria-hidden="false"/>', function ($node, options) {
    $node.html([
      ...
      (options.footer ?
      '    <div class="modal-footer"><button type="button" class="btn btn-default" onclick="$(\'.modal-closed\').modal(\'hide\')">取 消</button>' + options.footer + '</div>' : ''
      ),
      '  </div>',
      '</div>'
    ].join(''));
